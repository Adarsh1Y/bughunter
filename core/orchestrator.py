...
from agents.response.llm_analyzer import analyze_multi_user_with_llm, analyze_idor_with_llm
...
def run_idor_test(base_url: str, target: dict, user1: str = "user1", user2: str = "user2") -> dict:
    ...
    result = test_idor_vulnerability(base_url, endpoint, param, user1=user1, user2=user2)
...
def run_session_testing_menu(base_url: str, target: dict) -> dict:
    ...
    results = {"cross_user_test": None, "idor_test": None, "multi_user_analysis": None}
    ...
    if choice in ("1", "3"):
        ...
        results["cross_user_test"] = run_cross_user_test(
            base_url, target.get("endpoint", "/api/data"), "user1", "user2"
        )

    if choice in ("2", "3"):
        print("\n[IDOR Test]")
        results["idor_test"] = run_idor_test(base_url, target, "user1", "user2")
