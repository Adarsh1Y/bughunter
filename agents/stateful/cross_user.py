...
def test_same_endpoint_different_users(
    ...
    result = {
        ...
        "user1": {
            ...
            "length": resp1.get("length")
        },
        "user2": {
            "user": user2,
            "status": resp2.get("status"),
            "length": resp2.get("length")
        },
    ...
    if result["idor_detected"]:
        print(f"  [!] IDOR VULNERABILITY DETECTED")
...
def test_horizontal_escalation(
    ...
    return test_same_endpoint_different_users(
        base_url, resource, user1, user2,
        param="id", param_value="1"
    )
...
def test_vertical_escalation(
    ...
    result = {
        ...
        "comparison": comparison,
        "privesc_detected": (
            resp_regular.get("status") == 200 and 
            resp_admin.get("status") == 200
        )
    }
...
def test_batch_idor(
    ...
    results["summary"] = (
        f"Found {results['vulnerabilities_found']} vulnerabilities "
        f"out of {len(values)} tests"
    )
...
def test_multi_user_all_endpoints(
    base_url: str,
    endpoints: list,
    user1: str,
    user2: str
    ...
    print(f"\n[Summary] {results['vulnerabilities_found']}/{len(endpoints)} endpoints vulnerable")
