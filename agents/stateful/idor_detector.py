...
def detect_idor_params(endpoint: str, params: list) -> list:
    ...
    for param in params:
        ...
        if param_lower in IDOR_PATTERNS:
            ...
            idor_params.append(
                {"param": param, "type": "generic_id_reference", "risk": "high", "confidence": 0.75}
            )
...
def test_idor_vulnerability(
    ...
    if comparison1["issue"] == "idor" or comparison2["issue"] == "idor":
        ...
        print(f"  [!] IDOR POSSIBLE (Severity: {results['severity']})")
        ...
        if resp_user2_own.get("length", 0) > 0:
            ...
            results["explanation"] = (
                "Both users accessed same resource successfully - possible IDOR"
            )
...
def scan_endpoints_for_idor(base_url: str, endpoints: list, user1: str, user2: str) -> dict:
    """
    ...
    """
    results = {"endpoints_tested": len(endpoints), "vulnerabilities_found": 0, "findings": []}
    ...
    for endpoint in endpoints:
        if is_idor_suspect_endpoint(endpoint):
            ...
            if not idor_params:
                idor_params = [
                    {"param": "id", "type": "generic", "risk": "medium", "confidence": 0.7}
                ]
